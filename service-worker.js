...
const utils = (() => {
    ...
    const getFile = async filePath => {
        const value = (await changes.getItem(filePath) || {}).value || await files.getItem(filePath);
        try {
            return JSON5.parse(value);
        } catch (e) {}
        return value;
    }, rootFiles = [ [ "~/.git/config", "\n" ], [ "~/settings.json", settings() ], [ "~/.profile", profile() ], [ "~/welcome.md", welcome() ], [ "~/importmap.json", importmap() ] ];
    for (let i = 0, len = rootFiles.length; i < len; i++) {
        const [path, defaultContent] = rootFiles[i];
        await getFile(path) || await files.setItem(path, defaultContent);
    }
    return await changes.setItem(`state-${service.name}-opened`, [ {
        name: "welcome.md",
        order: 0
    } ]), service;
};
...
function consolePlugin({types: t}) {
    return {
        visitor: {
            CallExpression(path) {
                path.node.callee.property && "console" === path.node.callee.object.name && "log" === path.node.callee.property.name && path.replaceWith(t.callExpression(t.identifier("processWrite"), path.node.arguments));
            },
    ...
    return {
        visitor: {
            ImportDeclaration(path, state) {
                const {map: map} = state.opts;
                map.imports[path.node.source.value] ? path.node.source.value = map.imports[path.node.source.value] : path.node.source.value.startsWith("/") && (path.node.source.value = `${self.location.origin}${path.node.source.value}`);
            }
        }
    };
}
    ...
    return {
        visitor: {
            AwaitExpression(path) {
                const selfHooks = t.memberExpression(t.identifier("self"), t.identifier("hooks")), selfHookCount = t.memberExpression(selfHooks, t.identifier("length")), hookBlock = t.blockStatement([ t.expressionStatement(t.assignmentExpression("=", t.memberExpression(selfHooks, selfHookCount, !0), path.node.argument)), t.returnStatement(t.memberExpression(selfHooks, t.binaryExpression("-", selfHookCount, t.numericLiteral(1)), !0)) ]);
                path.node.argument = t.callExpression(t.arrowFunctionExpression([], hookBlock), []);
            },
...
Babel.registerPlugin("console", consolePlugin), Babel.registerPlugin("importMap", importsPlugin), 
Babel.registerPlugin("processExit", processExitPlugin), Babel.registerPlugin("@babel/syntax-import-assertions", babelPluginSyntaxImportAssertions);
...
async function getHandler$1(args) {
    const {stores: stores} = this, {path: path, query: query} = args, getFile = async filePath => {
        const value = (await stores.changes.getItem(filePath) || {}).value || await stores.files.getItem(filePath);
        try {
            return JSON5.parse(value);
        } catch (e) {}
        return value;
    }, content = await getFile(path);
    if (x = path, !new RegExp(".js$").test(x)) return content;
    var x;
