...
const getClientId = () => {
    if (clientId$1) return clientId$1;
    clientId$1 = createClientId();
    return clientId$1;
};
...
const newTree = ({service: service, treeState: treeState}, context) => {
 ...
 //should keep track of this instead of blindly returning
    ...
    const memoryHandler = action => ({target: target}) => {
        const {path: path} = tree.context(target);
...
const listeners = [ {
    ...
    handlers: [ mainListeners.pointerdown ]
} ];
